...
void compileAssignSt(void) {
  ...
  // Store the value from stack top to the address below it
  genST();
}
...
void compileIfSt(void) {
  ...
  if (lookAhead->tokenType == KW_ELSE) {
    // Jump over else part after then part
    jInstruction = genJ(DC_VALUE);
    
    // Update false jump to else part
    updateFJ(fjInstruction, getCurrentCodeAddress());
    
    eat(KW_ELSE);
    compileStatement();
    
    // Update jump to end of if statement
    updateJ(jInstruction, getCurrentCodeAddress());
  } else {
    // No else part, update false jump to end
    updateFJ(fjInstruction, getCurrentCodeAddress());
  }
}
...
void compileWhileSt(void) {
  ...
  // Remember the address of the condition
...
Type* compileExpression3(Type* argType1) {
  ...
  case SB_MINUS:
    ...
    argType2 = compileTerm();
    checkIntType(argType2);
    // Subtract the two values on stack
    genSB();
    resultType = compileExpression3(argType1);
    break;
...
Type* compileTerm2(Type* argType1) {
  ...
  case SB_TIMES:
    ...
    checkIntType(argType2);
    // Multiply the two values on stack
    genML();
  ...
  case SB_SLASH:
    ...
    argType2 = compileFactor();
    checkIntType(argType2);
    // Divide the two values on stack
    genDV();
    resultType = compileTerm2(argType1);
    break;
    // check the FOLLOW set
...
Type* compileFactor(void) {
  ...
  case TK_NUMBER:
    eat(TK_NUMBER);
    // Push the number constant onto stack
    genLC(currentToken->value);
    type = intType;
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    // Push the character constant onto stack
  ...
  case TK_IDENT:
    ...
    switch (obj->kind) {
    case OBJ_CONSTANT:
      // Push the constant value onto stack
      if (obj->constAttrs->value->type == TP_INT) {
	...
	genHL();
        ...
        // Push the variable value onto stack
